...
theorem cons_eq {α : Type} (a : α) (t : List α) : List.cons a t = List.cons a t :=
  by rfl

lemma nil_eq {α : Type} : (List.nil : List α) = (List.nil : List α) :=
  by rfl
...
--by inst_simp
  by
  intros
  rawEgg [cons_eq, nil_eq]

#print head_eq_of_cons_eq
